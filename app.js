...
function updateUILanguage() {
    ...
    document.getElementById('key1Title').innerText = t.key1Title;
...
function setLanguage(lang) {
    ...
    updateUILanguage();
    // Перезагрузить открытый контент при смене языка
    if (currentContent) {
...
// Обработчики переключателя языка (будут назначены после загрузки DOM)
...
function renderStepWithFullscreen(step, nextCallback, backCallback, homeCallback, startCallback) {
    ...
    if(step.type==='welcome' || step.type==='text') {
        ...
        // Локализация стандартных кнопок, если они не переведены в JSON
        ...
        if (step.btnText === '🖼️ Просмотреть картинки') btnText = currentLang === 'ru' ? '🖼️ Показать иллюстрации' : '🖼️ Show illustrations';
        ...
        const nextKey = step.nextKey, purchased = userStatus[nextKey];
        let priceText = '';
        if (nextKey === 'key3') priceText = currentLang === 'ru' ? '1390 ₽' : '$19';
        else if (nextKey === 'key4') priceText = currentLang === 'ru' ? '1890 ₽' : '$25';
        else priceText = currentLang === 'ru' ? '890 ₽' : '$12';
